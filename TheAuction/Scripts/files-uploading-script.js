...
function upload(files) {
    ...
    $.ajax({
        type: 'POST',
        url: '/Home/UploadFile', // URL на метод действия Upload контроллера HomeController
        data: formData,
        processData: false,
        contentType: false,
        beforeSend: function () {
            ...
            if (data.Error) {
                ...
                if (data.Files) {
                    ...
                    for (var i = 0; i < data.Files.length; i++) {
                        ...
                        document.getElementById("url").value = data.Files[i];
                        document.getElementById("del").value = data.Files[i];
